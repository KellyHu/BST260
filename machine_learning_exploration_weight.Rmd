---
title: "machine_learning_exploration_weight"
...
dat_trsf_nz_im %>% as.tibble() %>%
  ...
  geom_histogram() +
  theme_bw()
summary(dat_trsf_nz_im$weight)
```

Split data into train set and test set.
```{r}
set.seed(1234)
trainIndex <- createDataPartition(dat_trsf_nz_im$weight, p = .8, 
                                  list = FALSE, 
                                  times = 1)
head(trainIndex)
```
...
```{r, warning=TRUE}
set.seed(123)
models = c("knn","lm", "ridge", "lasso", "rf")
train.control = trainControl(method = "repeatedcv", number = 5, repeats = 10, verbose = FALSE)

n_models = length(models)
for (m in 1:n_models) {
    fit = train(weight ~ ., method = models[m], trControl = train.control, data = x)
...
models = c("knn", "rf", "glm", "lda",  "sparseLDA", "svmLinear", "svmRadial", "svmRadialWeights")
train.control = trainControl(method = "repeatedcv", number = 10, repeats = 10, verbose = FALSE)

n_models = length(models)

for (m in 1:n_models) {
    fit = train(weight ~ ., method = models[m], trControl = train.control, data = x.c)
    print(c(models[m], getTrainPerf(fit)))
